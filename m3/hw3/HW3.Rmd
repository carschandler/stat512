...
because it yields a smaller critical value: $B = `r crit_bonf` < `r w` = W$.
...
If the variables are independent, then their covariance is $\mathbf{0}$.
...
As stated in [1.b], for joint estimation of the *parameters*, the Bonferroni
interval is better because it yields a smaller critical value: $B = `r 
crit_bonf_params` < `r w` = W$. This is consistent across all models when we are
estimating the parameters/weights/coefficients simultaneously for a linear
model. That is, $W/B > 1$.
...
Were all three predictors significant, we would expect to see all three
predictors have nonzero values. If zero falls within in any of the intervals for
the parameters, then we cannot say that all three predictors have significant
impact on $Y$.

Since the interval for $\beta_2$ overlaps with $0$ in both intervals, which
indicates no impact, we cannot say that all three predictors have a
