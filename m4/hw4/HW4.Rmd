...
$SSR(X_2 | X_1)$ can be found using the equation:

$$
SSR(X_2 | X_1) = SSR(X_1, X_2) - SSR(X_1)
$$

We already found $SSR(X_1)$ above, and we can find $SSR(X_1, X_2)$ by summing
the `x1` and `x2` rows of the `Sum Sq` columns in the MLR ANOVA table above:
...
$$
SSR(X_2 | X_1) = `r ssr_x12` - `r ssr_x1` = `r ssr_x2_x1`
$$
...
SSR(X_2 | X_1) &= SSR(X_1, X_2, X_3) - SSR(X_1) - SSR(X_3 | X_1, X_2)
\end{aligned}
$$

We already know $SSR(X_1) = `r ssr_x1`$, and we can find $SSR(X_1, X_2, X_3)$ by
summing the `x1`, `x2`, `x3` rows of the `Sum Sq` of the MLR ANOVA table above:

```{r}
ssr_x123 <- sum(anova_mlr[c("x1", "x2", "x3"), "Sum Sq"])
```

$$
SSR(X_1, X_2, X_3) = `r ssr_x123`
$$

So then we just need $SSR(X_3 | X_1, X_2)$.

From Equation (7.15), we know that the general linear test statistic for the
hypotheses:
...
and this $F^*$ is the same value in the `x3` row of the `F value` column of the
MLR ANOVA table above:

```{r}
f_star_3 <- anova_mlr["x3", "F value"]
```

So $F^* = `r f_star_3`$. We can find $SSE(X_1, X_2, X_3)$ at the `Residuals` row
of the `Sum Sq` column of the MLR ANOVA table above:

```{r}
sse_x123 <- anova_mlr["Residuals", "Sum Sq"]
```

So $SSE(X_1, X_2, X_3) = `r sse_x123`$, and now we can solve for $SSR(X_3 | X_1,
X_2)$:

```{r}
n <- length(y)
ssr_x3_x12 <- f_star_3 * sse_x123 / (n - 4)
```

$$
\begin{aligned}
F^* &= \frac{SSR(X_3 | X_1, X_2)}{1} \div \frac{SSE(X_1, X_2, X_3)}{n-4} \\
SSR(X_3 | X_1, X_2) &= F^* \frac{SSE(X_1, X_2, X_3)}{n-4} \\
SSR(X_3 | X_1, X_2) &= `r f_star_3` \frac{`r sse_x123`}{`r n`-4} \\
SSR(X_3 | X_1, X_2) &= `r ssr_x3_x12` \\
\end{aligned}
$$

Finally, we can substitute this value to solve:

```{r}
ssr_x2_x1 <- ssr_x123 - ssr_x1 - ssr_x3_x12
```

$$
\begin{aligned}
SSR(X_2 | X_1) &= SSR(X_1, X_2, X_3) - SSR(X_1) - SSR(X_3 | X_1, X_2) \\
SSR(X_2 | X_1) &= `r ssr_x123` - `ssr_x1` - `ssr_x3_x12` \\
SSR(X_2 | X_1) &= \mathbf{`r ssr_x2_x1`} \\
\end{aligned}
$$
-->
...
We already found $SSR(X_1, X_2)$ in [1.a.iii] by summing the `x1` and `x2` rows of the `Sum
Sq` columns in the MLR ANOVA table above:

$$
SSR(X_1, X_2) = `r ssr_x12`
$$
...
F^* &= `r ssr_x2_x1` \div \frac{sse_x12}{`r n` - 3} \\
F^* &= `r f_star_x2_x1`
\end{aligned}
$$
...
F^* &= `r ssr_x1` \div \frac{sse_x1}{`r n` - 2} \\
F^* &= `r f_star_x1`
\end{aligned}
$$
