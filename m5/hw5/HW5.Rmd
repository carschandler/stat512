...
```{r}
region_map <- data.frame(ne = 1, nc = 2, s = 3, w = 4)
...
very high, indicating that it has no linear impact on $Y$, and it also pulls up
the p-value for the interaction parameter. On the other hand, the parameter for
infection has a high level of significance in the incorrect model, and all of
the parameters in the original model are significant at $\alpha = 0.1$.
...
are close, but we cannot make any statistical conclusions without performing a
...
The test has a p-value of $`r p` > 0.05 = \alpha$, so we fail to reject $H_0$
and we conclude that we cannot say the linear impact of infection on $Y$ for NE
...
$BIC_p$, $Y \sim X_1 + X_3$, was a very close second in all metrics, with $Y ~
